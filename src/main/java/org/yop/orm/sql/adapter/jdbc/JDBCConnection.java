...
public class JDBCConnection implements IConnection {
	...
	public IRequest prepare(Query query) {
		...
		try {
			if (idColumns.length > 0) {
				...
				statement = connection.prepareStatement(query.getSafeSql(), this.generatedKeyCommand(query));
			}
	...
	/**
	 * @return if {@link Query#askGeneratedKeys()} → {@link Statement#RETURN_GENERATED_KEYS}
	 *         else → {@link Statement#NO_GENERATED_KEYS}
	 */
	private int generatedKeyCommand(Query query) {
		return query.askGeneratedKeys() ? Statement.RETURN_GENERATED_KEYS : Statement.NO_GENERATED_KEYS;
	}
}
