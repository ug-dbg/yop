 ...
 * <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.1">RFC 2616, Section 14.1</a>
 * <p>
 * A port to Java of Joe Gregorio's MIME-Type Parser:
 * <a href="http://code.google.com/p/mimeparse">MIME-Type parser on Google code</a>
 * <p>
 * Ported by Tom Zellman <tzellman@gmail.com>.
 * <p>
 * This code was released under the terms of the
...
public final class MIMEParse {
	 ...
	 * <pre>('application', 'xhtml', {'q', '0.5'})</pre>
	 */
	private static ParseResults parseMimeType(String mimeType) {
		...
		// Java URLConnection class sends an Accept header
		// that includes a single "*" - Turn it into a legal wildcard.
		if (fullType.equals("*")) {
			fullType = "*/*";
		}
	 ...
	 * <pre>('application', '*', {'q', '0.5'})</pre>
	 * <p>
	 * In addition this function also guarantees that there is a value for 'q'
	 * in the params dictionary, filling it in with a proper default if necessary.
	 */
	 ...
	 * that have already been parsed by {@link #parseMediaRange(String)}.
	 * <br>
	 * Returns a tuple of the fitness value and the value of the 'q' quality parameter of the best match,
	 * or (-1, 0) if no match was found.
	 * <br>
	 * Just as for {@link #qualityParsed(String, Collection)}, 'parsed_ranges' must be a list of parsed media ranges.
	 */
	private static FitnessAndQuality fitnessAndQualityParsed(
		String mimeType,
	...
	/**
	 * Find the best match for a given mime-type against a list of ranges
	 * that have already been parsed by {@link #parseMediaRange(String)}.
	 * Returns the 'q' quality parameter of the best match, 0 if no match was found.
	 * <br>
	 * This function behaves the same as {@link #quality(String, String)} except that 'parsed_ranges' must be a list
	 ...
	 * The value of header must be a string that conforms to the format of the HTTP Accept: header.
	 * The value of 'supported' is a list of mime-types.
	 * <p>
	 * Example :
	 * <pre>
	 * {@code
	 * MimeParse.bestMatch(Arrays.asList(new String[]{"application/xbel+xml", "text/xml"}), "text/*;q=0.5,*; q=0.1")
	 * }
	 * </pre>
	 * Returns 'text/xml'
	 */
	public static String bestMatch(Collection<String> supported, String header) {
		...
		for (String r : StringUtils.split(StringUtils.trimToEmpty(header), ',')) {
